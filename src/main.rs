...
fn domain_pattern_matches(pattern: &str, host: &str) -> bool {
    ...
    // A plain domain matches itself and its subdomains, mirroring the Python PoC.
...
async fn run(args: RunArgs) -> Result<()> {
    let expected_uid = user_uid(&args.common.user)?;
    let actual_uid = unsafe { libc::geteuid() };
    if actual_uid == 0 {
        bail!(
            "refusing to run as root; run as --user {}",
            args.common.user
        );
    }
    ...
    let rules = Arc::new(RwLock::new(DomainRules::load(args.domains)?));
    let proxy = Arc::new(Proxy::parse(&args.proxy)?);
    let listener = TcpListener::bind((Ipv4Addr::LOCALHOST, args.common.port)).await?;
    eprintln!(
...
fn configure_nft(action: &str, common: Option<&Common>) -> Result<()> {
    ...
    let common = common.context("install arguments missing")?;
    let uid = user_uid(&common.user)?;
    ...
    let script = format!(
        "add table inet {NFT_TABLE}\n\
         ...
         add rule inet {NFT_TABLE} output tcp dport {{ 80, 443 }} redirect to :{}\n",
        common.port
    );
    ...
    println!(
        "nftables rules installed; excluded service user is {} (uid {uid})",
        common.user
    );
    Ok(())
}
...
async fn main() -> Result<()> {
    match Cli::parse().action {
        Action::Install(common) => configure_nft("install", Some(&common)),
        Action::Remove => configure_nft("remove", None),
